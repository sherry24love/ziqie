...
Route::rule('/', 'user.Generate/index');
...
Route::rule('cloudflare','front.Index/CloudFlare');
...
if(!$domain){
	$domain = Domain::select()->toArray();
	Cache::set('domain', $domain);
}
if (!empty($domain)) {
	foreach ($domain as $key => $value) {
		Route::domain('www.'.$value['domain'], 'front.Index/domain');
	}
}
